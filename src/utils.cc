...
// TODO(QQueke): Remove this from here and declare in place
std::array<char, 2> AlpnString = {'h', '3'};

// The protocol name used in the Application Layer Protocol Negotiation (ALPN).
const QUIC_BUFFER Alpn = {sizeof("h3") - 1,
                          reinterpret_cast<uint8_t *>(AlpnString.data())};
...
std::string GetSSLErrorMessage(int error) {
  ...
  switch (error) {
    ...
    case SSL_ERROR_SYSCALL: {
      uint32_t errCode = ERR_peek_last_error();
    ...
    case SSL_ERROR_SSL: {
      ...
      ERR_error_string_n(errCode, errorDetails.data(), errorDetails.size());
      errorMessage =
          "Low-level SSL library " + std::string(errorDetails.data());
      break;
    }
...
uint32_t DecodeHexBuffer(_In_z_ const char *HexBuffer,
                         ...
                         _Out_writes_to_(OutBufferLen, return)
                             uint8_t *OutBuffer) {
  auto HexBufferLen = static_cast<uint32_t>(strlen(HexBuffer) / 2);
...
//   }
//
//   char ClientRandomBuffer[(2 * sizeof((reinterpret_cast<QUIC_TLS_SECRETS
//   *>(0))
//                                           ->ClientRandom)) +
//                           1] = {0};
//
